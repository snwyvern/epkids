...
Im Backend steuert die Processengine gemeinsam mit der Integrationengine das Geschehen. Beide Komponenten werden, wie alles andere auch, über grafische Editoren gesteuert. Dadurch ist eine einfache Erstellung von Prozessen, Integrationsregeln, Datenmodellen, Eingabeformularen usw. möglich. Alle Aspekte des Systems sind aufeinander abgestimmt.
...
Prozesse (Workflows) werden in BPMN2 spezifiziert und in der Activiti-Prozessengine ausgeführt. Activiti kann Serviceaufgaben an die Integrationengine delegieren.
