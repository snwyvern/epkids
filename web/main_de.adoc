...
link:https://de.wikipedia.org/wiki/Apache_Camel[Camelrouten,window="_blank"]  sind die hauptsächlichen Verarbeitungseinheiten in *_simpl4_*. Sie spezifizieren den Datenfluss von Komponente zu Komponente. Außerdem stellt Camel eine Vielzahl an Protokollen und Komponenten zur Verfügung, um so zum Beispiel Daten aus beliebigen Quellen wie einem SAP-System, einer IBM AS/400 oder vielen anderen Systemen und Lösungen bereitzustellen
