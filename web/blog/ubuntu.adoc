...
```bash
mkdir /zpool/gopath/src/github.com/coreos/
cd /zpool/gopath/src/github.com/coreos/
git clone https://github.com/coreos/flannel.git
cd flannel
git checkout v0.7.0
CGO_ENABLED=1 make dist/flannel
...
FLANNEL_ETCD_KEY="/simpl4.org/network"
FLANNEL_OPTIONS=""
```
...
ExecStart=/usr/local/bin/flanneld -ip-masq=false -etcd-endpoints=${FLANNEL_ETCD} -etcd-prefix=${FLANNEL_ETCD_KEY} $FLANNEL_OPTIONS
Restart=on-failure
