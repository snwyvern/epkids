...
num=$(hostname | tail  -c +2)
ip link add veth-network type veth peer name br-veth-network
brctl addif weave br-veth-network

ip link add veth-mgmt type veth peer name br-veth-mgmt
brctl addif weave br-veth-mgmt

ip link add veth-neutron type veth peer name br-veth-neutron
brctl addif weave br-veth-neutron

ifconfig veth-network 10.2.0.${num}/16
ifconfig veth-mgmt 10.3.0.${num}/16
ifconfig veth-neutron 10.4.0.${num}/16

ifconfig weave 10.2.0.${num}/16
