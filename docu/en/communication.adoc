...
=== Remote Procedure Calls(RPC) over http(s) (service entrypoint)
...
NOTE: Each service defined in the serviceeditor  for "rpc over http", must begin with a "Direct" endpoint. image:docu/images/communication/rpcendpoint.png[]

===== Service entrypoint ( reachable via RPC over https(s),  depretecated name "direct endpoint")
...
. the place where the parameter is stored: header/property or body. see description of exchange.
